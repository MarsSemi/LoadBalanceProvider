(function () {
  ...
  const rows = [
    ...
    ["即時監看", "Live Monitor", "实时监控", "リアルタイム監視", "실시간 모니터링"],
    ["請求密度", "Request Density", "请求密度", "リクエスト密度", "요청 밀도"],
    ["觀察各帳號近期請求頻率與高、中、低複雜度分布。", "Monitor recent request rates and high, medium, and low complexity distribution for each account.", "观察各账号近期请求频率与高、中、低复杂度分布。", "アカウントごとの最近のリクエスト頻度と複雑度分布を監視します。", "계정별 최근 요청 빈도와 복잡도 분포를 모니터링합니다."],
    ["觀察視窗", "Observation Window", "观察窗口", "観測期間", "관찰 구간"],
    ["總請求", "Total Requests", "总请求", "総リクエスト", "총 요청"],
    ["每分鐘請求", "Requests per Minute", "每分钟请求", "1 分あたりのリクエスト", "분당 요청"],
    ["有流量帳號", "Active Accounts", "有流量账号", "トラフィックのあるアカウント", "활성 계정"],
    ["低需求", "Low Demand", "低需求", "低負荷", "낮은 요구"],
    ["中需求", "Medium Demand", "中需求", "中負荷", "중간 요구"],
    ["高需求", "High Demand", "高需求", "高負荷", "높은 요구"],
    ...
    ["可使用 API 金鑰或 MCP 專用金鑰，透過 Authorization: Bearer 或 X-API-Key 連線。MCP 金鑰僅可呼叫 MCP，登入金鑰無法呼叫 MCP。", "Use an API key or dedicated MCP key through Authorization: Bearer or X-API-Key. MCP keys can only call MCP, and login keys cannot call MCP.", "可使用 API 密钥或 MCP 专用密钥，通过 Authorization: Bearer 或 X-API-Key 连接。MCP 密钥仅可调用 MCP，登录密钥无法调用 MCP。", "API キーまたは MCP 専用キーを使用し、Authorization: Bearer または X-API-Key で接続できます。MCP キーは MCP のみ呼び出せ、ログインキーでは MCP を呼び出せません。", "API 키 또는 MCP 전용 키를 사용해 Authorization: Bearer 또는 X-API-Key로 연결할 수 있습니다. MCP 키는 MCP만 호출할 수 있고 로그인 키는 MCP를 호출할 수 없습니다."],
    ...
    ["輸出比分級門檻", "Output Ratio Thresholds", "输出比分级阈值", "出力比分類しきい値", "출력 비율 분류 기준"],
    ["低輸出上限（%）", "Low output upper limit (%)", "低输出上限（%）", "低出力の上限（%）", "낮은 출력 상한(%)"],
    ["中輸出上限（%）", "Medium output upper limit (%)", "中输出上限（%）", "中出力の上限（%）", "중간 출력 상한(%)"],
    ["輸出比為完成輸出 Token 除以估算輸入 Token。低輸出小於等於低輸出上限；中輸出高於低輸出上限且小於等於中輸出上限；高輸出則高於中輸出上限。低輸出上限必須小於中輸出上限，設定儲存後會立即套用至即時監看統計。", "Output ratio is completed output tokens divided by estimated input tokens. Low output is at or below the low limit; medium output is above the low limit and at or below the medium limit; high output is above the medium limit. The low limit must be below the medium limit. Saved values apply to live monitoring immediately.", "输出比为完成输出 Token 除以估算输入 Token。低输出小于等于低输出上限；中输出高于低输出上限且小于等于中输出上限；高输出则高于中输出上限。低输出上限必须小于中输出上限，保存后会立即套用到实时监控统计。", "出力比は完了した出力 Token を推定入力 Token で割った値です。低出力は低上限以下、中出力は低上限を超え中上限以下、高出力は中上限を超える範囲です。低上限は中上限より小さくする必要があり、保存後すぐにリアルタイム監視へ反映されます。", "출력 비율은 완료된 출력 Token을 추정 입력 Token으로 나눈 값입니다. 낮은 출력은 낮은 상한 이하, 중간 출력은 낮은 상한 초과 및 중간 상한 이하, 높은 출력은 중간 상한 초과입니다. 낮은 상한은 중간 상한보다 작아야 하며 저장 즉시 실시간 모니터링에 적용됩니다."],
    ["儲存門檻", "Save Thresholds", "保存阈值", "しきい値を保存", "기준 저장"],
    ...
    ["低輸出", "Low Output", "低输出", "低出力", "낮은 출력"],
    ["中輸出", "Medium Output", "中输出", "中出力", "중간 출력"],
    ["高輸出", "High Output", "高输出", "高出力", "높은 출력"],
    ...
    ["API 金鑰可呼叫一般 API 與 MCP；MCP 金鑰僅可呼叫 MCP。", "API keys can call general APIs and MCP; MCP keys can only call MCP.", "API 密钥可调用一般 API 与 MCP；MCP 密钥仅可调用 MCP。", "API キーは一般 API と MCP を呼び出せます。MCP キーは MCP のみ呼び出せます。", "API 키는 일반 API와 MCP를 호출할 수 있으며, MCP 키는 MCP만 호출할 수 있습니다."],
    ["金鑰類型", "Key type", "密钥类型", "キーの種類", "키 유형"],
    ["API 金鑰", "API Keys", "API 密钥", "API キー", "API 키"],
    ...
    ["API Key 名稱", "API Key Name", "API Key 名称", "API キー名", "API 키 이름"],
    ...
    ["核發 API Key", "Issue API Key", "签发 API Key", "API キーを発行", "API 키 발급"],
    ...
    ["其他", "Other", "其他", "その他", "기타"],
    ["請先輸入 API Key 名稱。", "Enter an API Key name first.", "请先输入 API Key 名称。", "先に API キー名を入力してください。", "먼저 API 키 이름을 입력하세요."],
    ...
    ["API Key 已核發，路由策略預設為 AUTO。", "API Key issued. The routing policy defaults to AUTO.", "API Key 已签发，路由策略默认为 AUTO。", "API キーを発行しました。ルーティングポリシーの既定値は AUTO です。", "API 키가 발급되었습니다. 라우팅 정책 기본값은 AUTO입니다."],
  ...
  rows.push(
    ...
    ["低輸出上限必須介於 0% 到 100%。", "The low output limit must be between 0% and 100%.", "低输出上限必须介于 0% 到 100%。", "低出力の上限は 0%～100% にしてください。", "낮은 출력 상한은 0%에서 100% 사이여야 합니다."],
    ["中輸出上限必須介於 0% 到 100%。", "The medium output limit must be between 0% and 100%.", "中输出上限必须介于 0% 到 100%。", "中出力の上限は 0%～100% にしてください。", "중간 출력 상한은 0%에서 100% 사이여야 합니다."],
    ["低輸出上限必須小於中輸出上限。", "The low output limit must be lower than the medium output limit.", "低输出上限必须小于中输出上限。", "低出力の上限は中出力の上限より小さくしてください。", "낮은 출력 상한은 중간 출력 상한보다 작아야 합니다."],
    ["輸出比分級門檻儲存中。", "Saving output ratio thresholds.", "正在保存输出比分级阈值。", "出力比分類しきい値を保存中です。", "출력 비율 분류 기준을 저장하는 중입니다."],
    ["輸出比分級門檻已儲存並立即生效。", "Output ratio thresholds saved and applied immediately.", "输出比分级阈值已保存并立即生效。", "出力比分類しきい値を保存し、すぐに反映しました。", "출력 비율 분류 기준을 저장하고 즉시 적용했습니다."],
    ["儲存輸出比分級門檻失敗。", "Failed to save output ratio thresholds.", "保存输出比分级阈值失败。", "出力比分類しきい値の保存に失敗しました。", "출력 비율 분류 기준 저장에 실패했습니다."],
    ...
    ["尚無可顯示的 API Key", "No API Keys to display", "暂无可显示的 API Key", "表示できる API キーはありません", "표시할 API 키가 없습니다"],
    ["未分類", "Uncategorized", "未分类", "未分類", "미분류"],
    ["未命名金鑰", "Unnamed Key", "未命名密钥", "名前のないキー", "이름 없는 키"],
    ["API Key 已核發，可呼叫一般 API 與 MCP。", "API Key issued. It can call general APIs and MCP.", "API Key 已签发，可调用一般 API 与 MCP。", "API キーを発行しました。一般 API と MCP を呼び出せます。", "API 키가 발급되었습니다. 일반 API와 MCP를 호출할 수 있습니다."],
    ...
    ["每日用量 = 當日各額度週期的使用量累計", "Daily usage = cumulative usage across the day's quota cycles", "每日用量 = 当日各额度周期的使用量累计", "日別使用量 = 当日の各クォータ周期の使用量累計", "일일 사용량 = 당일 각 할당량 주기의 누적 사용량"],
